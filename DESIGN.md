...
Manage is the **file cabinet**, titled ON FILE with the same tally as its page meta ("3 of 4 in the packet") and one quiet way back to the packet. `.cabinet` is `minmax(260px, 340px) minmax(0, 1fr)` with a 36px gutter: on the left an **index** of what is on file, four ink-ruled sections (CV, Job descriptions, Supporting docs, GitHub repos) each with an 11px caps head and a quiet Add, and under each an `.entry` (a button row: 14.5px name, 12.5px `--ink-2` meta) named by what the thing *is* - the candidate's name with the current role, the role @ company with its seniority and ACTIVE stamp, the doc's own title with where it was filed, the repo with its top tech - never by a filename, size or date. The open entry fills highlighter. On the right a **reading pane**: a boxed field labelled by kind (CV, JOB DESCRIPTION, SUPPORTING DOC, GITHUB REPO) whose head carries the file's provenance (filename, "Uploaded 28 Aug · 1,083 chars", a stamp only when embedding is pending or failed) and its actions (Replace CV with its consequence in 12px ink under it; Make active / Re-analyze / Delete; Remap / Delete), then a `.digest` of what prep read out of the file as `104px minmax(0, 1fr)` rows with 10px caps keys: the CV as the profile (Summary cut to two sentences, Roles, Skills as chips, Education), the JD as its opening line and the brief's runs, the doc as its opening, its section headings and where it was filed, the repo as tech and features. "Read the full text" swaps the digest for the file in the typed face (13px, pre-wrap, 560px max, scrolling). The selection lives in `?file=` so Back leaves the page. Under 980px the pane drops under the index; under 720px the pane head and digest rows stack. The danger zone stays a single ruled row at the foot.
...
Spacing rhythm: 4, 6, 8, 10, 12, 14, 16, 18, 22, 28. Row lists (jobs, docs, sessions, previous topics) use 10px vertical padding with a `--rule` divider and no side padding; hovered rows tint `--paper-2`. Modals are a `min(720px, 100%)` slip (the repo picker `min(880px, 100%)`) with `28px 32px 24px` padding over a 55% black backdrop; the slip keeps its title and its buttons in view and scrolls only its body.
...
### Lists and Rows
Job, document, session, previous-topic and manage rows: 10px vertical padding, `--rule` bottom divider, no side padding, `--paper-2` on hover. Selected rows (current job, chosen round type, the open file on Manage) fill highlighter and gain 10px side padding. Previous topics are `<details>` rows with a `82px 1fr auto` summary carrying the caps key, the truncated focus, and mini rating cells. A filed turn anywhere (previous topics, History records) is an `.exchange`: a `120px minmax(0,1fr)` grid, caps key (`--pen` for the interviewer's moves), then the words in the speaker's face. The live round's foot holds only the End session control (ghost, `--ink-2`); its consequence ("files what you have so far to History") is written only once the control is armed.

### Modal (slip)
`min(720px, 100%)` paper slip with grain and `--slip-shadow`, `28px 32px 24px`, title under a 2px ink rule, 30px square ink-outlined close button (hover inverts), footer over a 1px ink rule. Backdrop `rgba(0,0,0,.55)`. The count/context line sits under the title as meta, never above it. The slip is capped at the viewport less 48px and never scrolls as a whole: the header and footer stay in view and the body scrolls, so the action is always at hand on a long list. The repo picker is the wide slip (`min(880px, 100%)`): its filter stays put and only the `.repo-list` scrolls; each `.repo-row` is a `auto minmax(0, 1fr) auto` grid (checkbox, then the name at 14.5px 500 with its description wrapping under it in 13px `--ink-2` clamped to two lines, then the language chip, stars and "Pushed Mar 2026" in 12px `--ink-3` at the right), 10px vertical padding on a `--rule` divider, `--paper-2` on hover. Checking a repo draws the highlighter across its name (the 380ms swipe); the rows themselves never fill. Under 720px the tech line drops under the name and the footer stacks, primary on top.
...
- **Completion:** the round is closed three ways, in order, the first time the closed sheet arrives (`.practice-done.play`; a revisit shows the marks still): the highlighter is drawn across "Round complete." (the 380ms swipe, 300ms in); the panel's **stamp** (`.stamp`: `4px double currentColor`, 28px 700 caps 0.18em with the round's date in 12px under it, rotated -6deg, multiply on day / screen on night, 92% opacity behind the `--stamp-ink` noise mask so the ink did not take everywhere) slams at 420ms onto the free paper to the right of the closing lines (`.done-head`, a wrapping flex row: the title, score row, hint and button on the left, the stamp `margin-left: auto`, so on a narrow sheet it takes its own line, right-aligned, above the topics rule), 320ms from twice its size with a slight overshoot, and the block jolts 2px under it; COMPLETE in `--ok`, ABANDONED in `--pen` (`.bad`), the same fate the History stamp shows. At 1240ms the interviewer's **verdict** rises on the score row itself, beside "7.5 / 10 over 2 topics" with its arrow at the scale (a `PenNote` keyed VERDICT, `max-width: 36ch`, wrapping under the cells only on a phone) whose text comes from the scores alone (`verdict()` in `jobLabel.ts`: Strong / Solid / Keep at it for an average of 8 / 6 / below, then "Strongest on topic n, weakest on topic m" when the topics differ). Only a completed round gets the highlighter and the verdict; an abandoned one gets the red stamp. No particles, no library.
