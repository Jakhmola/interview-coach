...
The ready landing (Setup, once prep is complete) is the packet's cover. The header names the candidate and the role once (the ROLE / COMPANY field is the job switcher; no second role heading, no CV field). Then `.ready-actions`: the one action (Start a practice round) with, right-aligned, what is on file in a 13px `--ink-2` line ("On file: cv.pdf · 1 supporting doc · no repos") and a quiet Manage - the inventory itself lives on Manage. Then `.brief`, three `.section`s in `repeat(auto-fit, minmax(340px, 1fr))` columns with a `22px 40px` gap, each a caps head over `.brief-row`s (`104px minmax(0, 1fr)`, 13.5px, a 10px caps key at the left): **Role brief** as the JD analysis read it (seniority; must-haves and nice-to-haves as plain `.lines`, since they come back as sentences; duties as an unticked `.checklist`; behavioral signals as `.chips`), **Company notes** from research (the mission as a `.brief-lede` clamped to 4 lines, products and values as chips, recent news as lines, a hint with the source count and date; or one `--ink-2` line saying research came up empty and where to retry), and **Candidate profile** as the profile builder read the CV (the summary clamped, skills as chips capped at 14 with "+n more", experiences and projects as lines). Everything on the cover is what the questions are grounded in, so the candidate sees the packet the interviewer sees. Last, **Rounds for this role**: up to four `.round-row`s (round type, date and topic count, the status stamp pushed right, Resume for an active one) with "All records" to History, or "None yet". Under 720px the brief is one column and the on-file line drops under the button.
